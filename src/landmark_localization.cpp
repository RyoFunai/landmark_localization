...
namespace landmark_localization
{
      ...
      : rclcpp::Node("landmark_localization", name_space, options)
  {
    ...
    // image_processing(intrinsics, pipe_);
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    rs2::pipeline pipe;
    pipe.start();
    ...
    while (true)
    {
      ...
      {
        RCLCPP_WARN(this->get_logger(), "フレームの取得に失敗しました。タイムアウトしました。");
        continue;
      }
    }
  }

  void LandmarkLocalization::image_processing(rs2_intrinsics &intrinsics, rs2::pipeline &pipe_)
  {
    while (true)
    {
      RCLCPP_INFO(this->get_logger(), "0");
